...
def main():
	...
	def transform_raw_data(path):
		...
		infos.columns = [ 'AGE', 'GENDER','FAMILY STATUS', 
		               ...
		               'GOODS PRICE', 'AMOUNT CREDIT', 'YEARLY INCOME', 'AMOUNT ANNUITY', 
		             ]
		return infos
	...
	fig = go.Figure(data=[
	    go.Bar(name='Client sélectionné', x=bar_cols, y=infos.loc[client_id, bar_cols].values, marker_color='blue'),
	    go.Bar(name='Moyenne des clients', x=bar_cols, y=infos.loc['Moyenne clients', bar_cols].values, marker_color='orange')
	])
